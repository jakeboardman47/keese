...
func makeCRA(name, fromTenant, toTenant string) *authzv1alpha1.CrossTenantAgreement {
	return &authzv1alpha1.CrossTenantAgreement{
		ObjectMeta: metav1.ObjectMeta{
			Name: name,
		},
		Spec: authzv1alpha1.CrossTenantAgreementSpec{
			From: authzv1alpha1.TenantEndpoint{
				TenantRef: authzv1alpha1.LocalObjectRef{Name: fromTenant},
			},
			To: authzv1alpha1.TenantEndpoint{
				TenantRef: authzv1alpha1.LocalObjectRef{Name: toTenant},
			},
			Scope: authzv1alpha1.CRAScope{
				NATSSubjects: []string{"keese.cta.test"},
				A2ARoles:     []authzv1alpha1.A2ARole{authzv1alpha1.A2ARoleReader},
			},
		},
	}
}
...
func makeCRAReconciler(rebac CTARebacWriter, cosign CosignVerifier, saToken SATokenHmacVerifier, nats NatsStreamDeleter) *CrossTenantAgreementReconciler {
	if rebac == nil {
		rebac = &CTAFakeRebacWriter{}
	}
	if cosign == nil {
...
// sigType should be string(authzv1alpha1.SignatureTypeSAToken) or OIDCKeyless.
func driveToApproved(name, fromT, toT, sigType string, r *CrossTenantAgreementReconciler) {
	...
	var mid authzv1alpha1.CrossTenantAgreement
...
var _ = Describe("CrossTenantAgreementReconciler", func() {
	...
	Describe("TestReconcileIdempotent_CrossTenantAgreement", func() {
		It("converges in ≤3 reconciles with no spec change (rule 04-kubernetes.md §16)", func() {
			...
			for i := 0; i < 3; i++ {
				...
				var fresh authzv1alpha1.CrossTenantAgreement
	...
	Describe("Bilateral approval: OIDC-keyless (cosign) happy path", func() {
		It("transitions to Approved and writes ReBAC tuples after both cosign approvals", func() {
			...
			fakeRebac := &CTAFakeRebacWriter{}
			recorder := &craCapturingRecorder{}
			r := makeCRAReconciler(fakeRebac, &FakeCosignVerifier{}, nil, nil)
			r.Recorder = recorder

			driveToApproved(name, fromT, toT, string(authzv1alpha1.SignatureTypeOIDCKeyless), r)

			var fresh authzv1alpha1.CrossTenantAgreement
			Expect(k8sClient.Get(ctx, craNSN(name), &fresh)).To(Succeed())
			Expect(fresh.Status.Phase).To(Equal(authzv1alpha1.CRAPhaseA))
			...
			for _, appr := range fresh.Status.Approvals {
				Expect(appr.SignatureType).To(Equal(authzv1alpha1.SignatureTypeOIDCKeyless))
			}
		})
	})
	...
	Describe("Bilateral approval: SA-token happy path", func() {
		It("transitions to Approved after both tenants sign with SA-token HMAC", func() {
			...
			driveToApproved(name, fromT, toT, string(authzv1alpha1.SignatureTypeSAToken), r)
			...
			Expect(fresh.Status.Approvals).To(HaveLen(2))
			for _, appr := range fresh.Status.Approvals {
				Expect(appr.SignatureType).To(Equal(authzv1alpha1.SignatureTypeSAToken))
			}
		})
	})
	...
	Describe("Signature verification failure", func() {
		It("emits CRAApprovalInvalid and keeps phase Pending when cosign verify fails", func() {
			...
			annotateCRA(name, approveAnnotations(fromT, "malicious@example.com", "bad-sig", string(authzv1alpha1.SignatureTypeOIDCKeyless)))
			...
			var fresh authzv1alpha1.CrossTenantAgreement
			Expect(k8sClient.Get(ctx, craNSN(name), &fresh)).To(Succeed())
			Expect(fresh.Status.Phase).To(Equal(authzv1alpha1.CRAPhaseP))
	...
	Describe("TOFU snapshot frozen on Approved transition", func() {
		It("freezes workspace snapshot with non-zero SnapshotAt on Approved transition", func() {
			...
			var fresh2 authzv1alpha1.CrossTenantAgreement
	...
	Describe("Snapshot drift detection", func() {
		It("emits WorkspaceSnapshotDrift when snapshot diverges from current selector", func() {
			...
			var approved authzv1alpha1.CrossTenantAgreement
			Expect(k8sClient.Get(ctx, craNSN(name), &approved)).To(Succeed())
			Expect(approved.Status.Phase).To(Equal(authzv1alpha1.CRAPhaseA))
			...
			approved.Status.WorkspaceSnapshot = []authzv1alpha1.WorkspaceSnapshotEntry{
	...
	Describe("Expiry transition", func() {
		It("transitions to Expired and deletes ReBAC tuples when expiresAt is in the past", func() {
			...
			var current authzv1alpha1.CrossTenantAgreement
			Expect(k8sClient.Get(ctx, craNSN(name), &current)).To(Succeed())
			orig := current.DeepCopy()
			current.Status.Phase = authzv1alpha1.CRAPhaseA
			current.Status.WorkspaceSnapshot = []authzv1alpha1.WorkspaceSnapshotEntry{
			...
			var fresh authzv1alpha1.CrossTenantAgreement
			Expect(k8sClient.Get(ctx, craNSN(name), &fresh)).To(Succeed())
			Expect(fresh.Status.Phase).To(Equal(authzv1alpha1.CRAPhaseE),
	...
	Describe("Conflict detection", func() {
		It("emits CRAConflict and moves to Rejected when an Approved CRA covers the same tenant pair", func() {
			...
			Eventually(func(g Gomega) {
				...
				g.Expect(fresh.Status.Phase).NotTo(Equal(authzv1alpha1.CrossTenantAgreementPhase("")),
					"manager must have reconciled existing CRA and set an initial phase")
				orig := fresh.DeepCopy()
				fresh.Status.Phase = authzv1alpha1.CRAPhaseA
				...
				g.Expect(k8sClient.Get(ctx, craNSN(existingName), &fresh)).To(Succeed())
				g.Expect(fresh.Status.Phase).To(Equal(authzv1alpha1.CRAPhaseA))
			...
			Eventually(func(g Gomega) {
				...
				g.Expect(fresh.Status.Phase).To(Equal(authzv1alpha1.CRAPhaseR),
	...
	Describe("Out-of-band tuple idempotency", func() {
		It("is a no-op and reaches Approved even when ReBAC Sync is called for already-present tuples", func() {
			// The CTAFakeRebacWriter.Sync is idempotent by contract: calling it with
			...
			fakeRebac.Synced = append(fakeRebac.Synced,
				...
				CTARebacTuple{Object: "workspace:ws-" + toT, Relation: "messageable_from", User: "workspace:ws-" + fromT},
			)
