...
var _ = BeforeSuite(func() {
	...
	// Start a controller manager so both reconcilers run against envtest.
	...
	err = (&CrossTenantAgreementReconciler{
		Client:          mgr.GetClient(),
		Scheme:          mgr.GetScheme(),
		Recorder:        mgr.GetEventRecorderFor("crosstenanagreement-controller"),
		Rebac:           fakeRebac,
		CosignVerifier:  &FakeCosignVerifier{},
		SATokenVerifier: &FakeSATokenHmacVerifier{},
		NatsDeleter:     &FakeNatsStreamDeleter{},
	}).SetupWithManager(mgr)
	Expect(err).NotTo(HaveOccurred())
