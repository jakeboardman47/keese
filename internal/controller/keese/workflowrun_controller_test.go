...
func minimalWorkflowRun(name, workflowName string) *keesev1alpha1.WorkflowRun {
	return &keesev1alpha1.WorkflowRun{
		...
		Spec: keesev1alpha1.WorkflowRunSpec{
			...
			WorkflowRef:  keesev1alpha1.LocalObjectReference{Name: workflowName},
			RetryBudget:  10,
		},
	}
}
...
func setupWorkflow(ctx context.Context, name string, argo *FakeArgoProjector, rebac *WorkflowFakeRebacWriter) *keesev1alpha1.Workflow {
...
func wfrController(argo *FakeArgoProjector, nats *FakeNatsStreamProvisioner, natsDel *FakeNatsStreamDeleter,
	rebac *WorkflowFakeRebacWriter, cta *FakeWorkflowCTAResolver) *WorkflowRunReconciler {
...
var _ = Describe("WorkflowRun Controller", func() {
	Describe("Idempotency", func() {
		It("converges in ≤3 reconciles with no spec change", func() {
			...
			defer func() {
				_ = k8sClient.Delete(ctx, &keesev1alpha1.WorkflowRun{
			...
			var fetched keesev1alpha1.WorkflowRun
			Expect(k8sClient.Get(ctx, key, &fetched)).To(Succeed())
			Expect(fetched.Status.Phase).NotTo(Equal(keesev1alpha1.WorkflowRunPhaseError))
	...
	Describe("NATS stream provisioning", func() {
		It("provisions a NATS stream on create and deletes it on finalizer cleanup", func() {
			...
			Eventually(func() bool {
				err := k8sClient.Get(ctx, key, &keesev1alpha1.WorkflowRun{})
				return err != nil
	...
	Describe("Argo phase back-projection", func() {
		It("mirrors Argo Workflow phase to WorkflowRun.status", func() {
			...
			Expect(fetched.Status.Phase).To(Equal(keesev1alpha1.WorkflowRunPhaseRunning))
		})
	})
	...
	Describe("CrossTenantAgreement admission check", func() {
		It("keeps phase Pending when CTA is missing", func() {
			...
			var fetched keesev1alpha1.WorkflowRun
			Expect(k8sClient.Get(ctx, key, &fetched)).To(Succeed())
			Expect(fetched.Status.Phase).To(Equal(keesev1alpha1.WorkflowRunPhasePending))
		})
		...
		It("advances phase when CTA is Approved", func() {
			...
			Expect(fetched.Status.Phase).NotTo(Equal(keesev1alpha1.WorkflowRunPhasePending))
	...
	Describe("Retry budget composition", func() {
		It("uses min(wf.defaultRetryBudget, wfr.retryBudget) in Argo projection", func() {
			...
			wf.Spec.DefaultRetryBudget = &keesev1alpha1.RetryBudget{Limit: 3}
			Expect(k8sClient.Create(ctx, wf)).To(Succeed())
			defer func() {
				_ = k8sClient.Delete(ctx, &keesev1alpha1.Workflow{
					ObjectMeta: metav1.ObjectMeta{Name: wf.Name, Namespace: wf.Namespace},
	...
	Describe("ConcurrencyPolicy: Forbid", func() {
		It("keeps second WorkflowRun in Pending phase", func() {
			...
			wf.Spec.ConcurrencyPolicy = keesev1alpha1.ConcurrencyPolicyForbid
			Expect(k8sClient.Create(ctx, wf)).To(Succeed())
			defer func() {
				_ = k8sClient.Delete(ctx, &keesev1alpha1.Workflow{
					ObjectMeta: metav1.ObjectMeta{Name: wf.Name, Namespace: wf.Namespace},
	...
	Describe("Supervision context annotation propagation", func() {
		It("propagates supervision labels to the Argo Workflow projection", func() {
			...
			wfr.Spec.SupervisionContext = &keesev1alpha1.SupervisionContext{
