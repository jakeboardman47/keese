...
var _ = Describe("RuntimeExtension Controller", func() {
	...
	newExtReconciler := func(rebac RuntimeRebacWriter) *RuntimeExtensionReconciler {
	...
	createGooseRuntime := func(name string) *keesev1alpha1.AgentRuntime {
		ar := &keesev1alpha1.AgentRuntime{
			...
			Spec: keesev1alpha1.AgentRuntimeSpec{
				Implementation: keesev1alpha1.AgentRuntimeImplementation{
					Goose: &keesev1alpha1.GooseSpec{Image: "ghcr.io/goose:latest"},
				},
			},
		}
	...
	Describe("RuntimeExtension_owner_ref_to_agentruntime", func() {
		It("resolves runtimeRef and writes owner tuple on create", func() {
			...
			fake := NewRuntimeFakeRebacWriter()
			...
			ext := &keesev1alpha1.RuntimeExtension{
				...
				Spec: keesev1alpha1.RuntimeExtensionSpec{
					RuntimeRef:  keesev1alpha1.RuntimeRef{Name: ar.Name},
			...
			fetched := &keesev1alpha1.RuntimeExtension{}
			Expect(k8sClient.Get(ctx, nn, fetched)).To(Succeed())
			Expect(fetched.Status.Phase).To(Equal(keesev1alpha1.RuntimeExtensionPhaseReady))
		})
	})
	...
	Describe("RuntimeExtension_runtimeref_invalid_degrades", func() {
		It("sets Degraded phase and emits ExtensionRuntimeRefInvalid when runtimeRef missing", func() {
			...
			ext := &keesev1alpha1.RuntimeExtension{
				...
				Spec: keesev1alpha1.RuntimeExtensionSpec{
					RuntimeRef: keesev1alpha1.RuntimeRef{Name: "does-not-exist"},
				},
			}
			Expect(k8sClient.Create(ctx, ext)).To(Succeed())
			DeferCleanup(func() { _ = k8sClient.Delete(context.Background(), ext) })
			...
			Expect(fetched.Status.Phase).To(Equal(keesev1alpha1.RuntimeExtensionPhaseDegraded))
		})
	})
