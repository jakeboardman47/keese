...
func (r *MemoryReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	mem.Status.Phase = keesev1alpha1.MemoryPhaseProvisioning
	...
	// 8. ReBAC: write owner tuple. MemoryTuple purge on delete (step 3) is ordered BEFORE
	//    backend deprovision to prevent orphaned grants.
	ownerTuple := MemoryOwnerTuple(string(mem.UID), mem.Spec.WorkspaceRef)
	if err := r.Rebac.Write(ctx, []MemoryTuple{ownerTuple}); err != nil {
	...
	return r.setStatus(ctx, mem, orig, keesev1alpha1.MemoryPhaseReady,
		metav1.Condition{
			Type:    "Ready",
...
func (r *MemoryReconciler) reconcileDelete(ctx context.Context, mem, orig *keesev1alpha1.Memory) (ctrl.Result, error) {
	log := logf.FromContext(ctx)
	...
	mem.Status.Phase = keesev1alpha1.MemoryPhaseTerminating
	...
	ownerTuple := MemoryOwnerTuple(string(mem.UID), mem.Spec.WorkspaceRef)
	if err := r.Rebac.Delete(ctx, []MemoryTuple{ownerTuple}); err != nil {
		...
		return r.setStatus(ctx, mem, orig, keesev1alpha1.MemoryPhaseTerminating,
			metav1.Condition{
				Type:    "Ready",
				Status:  metav1.ConditionFalse,
