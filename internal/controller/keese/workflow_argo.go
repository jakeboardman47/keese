...
type ArgoProjector interface {
	...
	ProjectWorkflowTemplate(ctx context.Context, wf *keesev1alpha1.Workflow) (string, error)

	// DeleteWorkflowTemplate removes the Argo WorkflowTemplate for a Workflow.
	// Returns nil if already absent (idempotent).
	DeleteWorkflowTemplate(ctx context.Context, wf *keesev1alpha1.Workflow) error
...
type FakeArgoProjector struct {
	...
	ProjectedTemplates []*keesev1alpha1.Workflow
	// DeletedTemplates accumulates DeleteWorkflowTemplate calls.
	DeletedTemplates []*keesev1alpha1.Workflow
...
func (f *FakeArgoProjector) ProjectWorkflowTemplate(_ context.Context, wf *keesev1alpha1.Workflow) (string, error) {
	if f.Err != nil {
		return "", f.Err
	}
...
func (f *FakeArgoProjector) DeleteWorkflowTemplate(_ context.Context, wf *keesev1alpha1.Workflow) error {
	if f.Err != nil {
		return f.Err
	}
