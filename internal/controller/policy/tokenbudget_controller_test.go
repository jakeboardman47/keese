...
func newTokenBudget(name, ns string) *policyv1alpha1.TokenBudget {
	return &policyv1alpha1.TokenBudget{
		ObjectMeta: metav1.ObjectMeta{
			Name:      name,
			Namespace: ns,
		},
		Spec: policyv1alpha1.TokenBudgetSpec{
			Scope: policyv1alpha1.TokenBudgetScope{
				Tenant: &policyv1alpha1.TokenBudgetScopeRef{Name: "test-tenant"},
			},
			WindowDuration: "1h",
			ExhaustionMode: policyv1alpha1.ExhaustionModeHard,
			Limits: []policyv1alpha1.TokenLimit{
				{
					Model:       "gpt-4",
...
var _ = Describe("TokenBudget Controller", func() {
	...
	Context("Idempotency", func() {
		...
		It("should produce identical status across 3 reconcile cycles with no spec change", func() {
			...
			var firstStatus policyv1alpha1.TokenBudgetStatus
			...
			var secondStatus policyv1alpha1.TokenBudgetStatus
			var tbSecond policyv1alpha1.TokenBudget
	...
	Context("Prometheus happy path", func() {
		...
		It("should set phase=Ready and consumedCurrent when Prometheus returns values below limit", func() {
			...
			}, 15*time.Second, 250*time.Millisecond).Should(Equal(policyv1alpha1.TokenBudgetPhaseReady))
	...
	Context("BudgetExceeded transition", func() {
		...
		It("should emit BudgetExceeded event and write NATS KV true when consumed >= limit", func() {
			...
			Eventually(func() policyv1alpha1.TokenBudgetPhase {
				...
				return t.Status.Phase
			}, 15*time.Second, 250*time.Millisecond).Should(Equal(policyv1alpha1.TokenBudgetPhaseExhausted))
		})
	...
	Context("Window reset", func() {
		...
		It("should archive consumedCurrent to consumedPrevious, zero current, clear NATS, and emit BudgetReset", func() {
			...
			var latest policyv1alpha1.TokenBudget
	...
	Context("Scale-down clamp", func() {
		It("should return 0 remaining when consumed exceeds limit (unit test for computeRemaining)", func() {
			...
			limit := policyv1alpha1.TokenLimit{
				Model:       "gpt-4",
				TotalTokens: ptr(1000),
			}
			consumed := policyv1alpha1.TokenUsageEntry{
				Model:       "gpt-4",
	...
	Context("resolveScope", func() {
		It("should return tenant scope", func() {
			r := &TokenBudgetReconciler{}
			tb := &policyv1alpha1.TokenBudget{
				Spec: policyv1alpha1.TokenBudgetSpec{
					Scope: policyv1alpha1.TokenBudgetScope{
						Tenant: &policyv1alpha1.TokenBudgetScopeRef{Name: "acme"},
					},
				},
			}
		...
		It("should return workspace scope", func() {
			...
			tb := &policyv1alpha1.TokenBudget{
				Spec: policyv1alpha1.TokenBudgetSpec{
					Scope: policyv1alpha1.TokenBudgetScope{
						Workspace: &policyv1alpha1.TokenBudgetScopeRef{Name: "ws-1"},
					},
				},
			}
			scopeType, scopeID := r.resolveScope(tb)
