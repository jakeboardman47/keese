...
func (r *WorkflowRunReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// 1. Fetch
	var wfr workflowv1alpha1.WorkflowRun
	...
	if err := r.Get(ctx, types.NamespacedName{
		...
		if errors.IsNotFound(err) {
			wfr.Status.Phase = workflowv1alpha1.WorkflowRunPhaseError
	...
	if wfr.Status.ArgoWorkflowName == "" {
		...
		wfr.Status.Phase = workflowv1alpha1.WorkflowRunPhaseProvisioning
	...
	if wfr.Status.Phase == workflowv1alpha1.WorkflowRunPhaseRunning ||
		wfr.Status.Phase == workflowv1alpha1.WorkflowRunPhaseProvisioning {
		return ctrl.Result{RequeueAfter: requeueAfterDuration}, nil
	}
...
func (r *WorkflowRunReconciler) checkConcurrency(
	...
	orig *workflowv1alpha1.WorkflowRun,
) (requeue bool, err error) {
	if wf.Spec.ConcurrencyPolicy == workflowv1alpha1.ConcurrencyPolicyAllow || wf.Spec.ConcurrencyPolicy == "" {
		return false, nil
	}

	// Check for an active run that is not this run.
	var runList workflowv1alpha1.WorkflowRunList
	...
	for _, run := range runList.Items {
		...
		if phase == workflowv1alpha1.WorkflowRunPhaseSucceeded ||
			phase == workflowv1alpha1.WorkflowRunPhaseFailed ||
			phase == workflowv1alpha1.WorkflowRunPhaseError {
			continue
		}

		switch wf.Spec.ConcurrencyPolicy {
		case workflowv1alpha1.ConcurrencyPolicyForbid:
			r.emitRunEvent(wfr, "Warning", ReasonConcurrentRunForbidden,
				"ConcurrencyPolicy=Forbid: run %s is still active", run.Name)
			wfr.Status.Phase = workflowv1alpha1.WorkflowRunPhasePending
		...
		case workflowv1alpha1.ConcurrencyPolicyReplace:
			...
			runCopy.Status.Phase = workflowv1alpha1.WorkflowRunPhaseFailed
...
func (r *WorkflowRunReconciler) ensureNATSStream(ctx context.Context, wfr *workflowv1alpha1.WorkflowRun) (string, error) {
...
func (r *WorkflowRunReconciler) projectArgoWorkflow(ctx context.Context, wf *workflowv1alpha1.Workflow, wfr *workflowv1alpha1.WorkflowRun) (string, error) {
...
func (r *WorkflowRunReconciler) syncArgoStatus(ctx context.Context, wfr *workflowv1alpha1.WorkflowRun) error {
	if wfr.Status.ArgoWorkflowName == "" {
		return nil
	}
	...
	// Mirror nodes.
	nodes := make([]workflowv1alpha1.NodeStatus, 0, len(argoStatus.Nodes))
	for _, n := range argoStatus.Nodes {
		nodes = append(nodes, workflowv1alpha1.NodeStatus{
			ID:          n.ID,
			Phase:       n.Phase,
	...
	// Mirror artifacts.
	artifacts := make([]workflowv1alpha1.ArtifactOutput, 0, len(argoStatus.Artifacts))
	for _, a := range argoStatus.Artifacts {
		artifacts = append(artifacts, workflowv1alpha1.ArtifactOutput{
			Name:   a.Name,
			Path:   a.Path,
			NodeID: a.NodeID,
		})
...
func (r *WorkflowRunReconciler) reconcileRunDelete(ctx context.Context, wfr *workflowv1alpha1.WorkflowRun) (ctrl.Result, error) {
	log := logf.FromContext(ctx)
...
func (r *WorkflowRunReconciler) patchRunStatus(ctx context.Context, wfr *workflowv1alpha1.WorkflowRun, orig *workflowv1alpha1.WorkflowRun) error {
	return r.Status().Patch(ctx, wfr, client.MergeFrom(orig))
}

// setRunCondition upserts a condition on wfr.Status.Conditions.
func (r *WorkflowRunReconciler) setRunCondition(wfr *workflowv1alpha1.WorkflowRun, condType string, status metav1.ConditionStatus, reason, msg string) {
...
func (r *WorkflowRunReconciler) emitRunEvent(wfr *workflowv1alpha1.WorkflowRun, eventtype, reason, messageFmt string, args ...interface{}) {
