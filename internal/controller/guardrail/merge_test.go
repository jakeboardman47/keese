...
func makeBinding(name string, allow, deny []string, budget *guardrailv1alpha1.TokenBudget, rl *guardrailv1alpha1.RateLimit) *guardrailv1alpha1.GuardrailBinding {
	b := &guardrailv1alpha1.GuardrailBinding{
		ObjectMeta: metav1.ObjectMeta{Name: name, Namespace: "default"},
	}
	tools := &guardrailv1alpha1.ToolsSpec{
		Allow:     allow,
		Deny:      deny,
		RateLimit: rl,
	}
...
func TestMergeBindings(t *testing.T) {
	...
	tests := []struct {
		name        string
		bindings    []*guardrailv1alpha1.GuardrailBinding
		wantAllow   []string
		wantDeny    []string
		wantInput   int64
		wantTotal   int64
		...
		},
		{
			...
			bindings: []*guardrailv1alpha1.GuardrailBinding{
				makeBinding("b0", []string{"tool_a", "tool_b"}, []string{"tool_bad"}, &guardrailv1alpha1.TokenBudget{Input: 1000, Total: 2000}, nil),
			},
		...
		},
		{
			...
			bindings: []*guardrailv1alpha1.GuardrailBinding{
				makeBinding("cluster", nil, nil, &guardrailv1alpha1.TokenBudget{Input: 5000, Output: 5000, Total: 10000}, nil),
				makeBinding("tenant", nil, nil, &guardrailv1alpha1.TokenBudget{Input: 3000, Output: 6000, Total: 8000}, nil),
			},
			wantInput: 3000,
			wantTotal: 8000,
		},
		...
		},
		{
			...
			bindings: []*guardrailv1alpha1.GuardrailBinding{
				makeBinding("cluster", nil, nil, nil, &guardrailv1alpha1.RateLimit{Requests: 100, Window: "1m", Scope: "tenant"}),
				makeBinding("workspace", nil, nil, nil, &guardrailv1alpha1.RateLimit{Requests: 30, Window: "1m", Scope: "workspace"}),
			},
			wantReqRate: 30,
		},
		{
		...
		},
		{
			...
			bindings: []*guardrailv1alpha1.GuardrailBinding{
				makeBinding("cluster", []string{"tool_a", "tool_b", "tool_c"}, []string{"bad_1"}, &guardrailv1alpha1.TokenBudget{Input: 10000, Total: 20000}, nil),
				makeBinding("tenant", []string{"tool_a", "tool_b"}, []string{"bad_2"}, &guardrailv1alpha1.TokenBudget{Input: 7000, Total: 15000}, nil),
				makeBinding("workspace", []string{"tool_a"}, []string{"bad_3"}, &guardrailv1alpha1.TokenBudget{Input: 5000, Total: 10000}, nil),
			},
