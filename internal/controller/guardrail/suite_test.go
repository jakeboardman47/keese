...
var _ = BeforeSuite(func() {
	...
	var err error
	err = guardrailv1alpha1.AddToScheme(scheme.Scheme)
	...
	// See workspace/suite_test.go for the root-cause analysis.
	crdBasePath := filepath.Join("..", "..", "..", "config", "crd", "bases")
	guardrailCRDs := []string{
		"guardrail.operator.keese.ai_guardrailbindings.yaml",
	}
	crdPaths := make([]string, 0, len(guardrailCRDs))
	for _, f := range guardrailCRDs {
