...
func makeClusterBinding(name, ns string, allow, deny []string) *guardrailv1alpha1.GuardrailBinding {
	b := &guardrailv1alpha1.GuardrailBinding{
		ObjectMeta: metav1.ObjectMeta{
			Name:      name,
			Namespace: ns,
			Labels:    managedLabels(),
		},
		Spec: guardrailv1alpha1.GuardrailBindingSpec{
			Scope: guardrailv1alpha1.BindingScope{Type: guardrailv1alpha1.BindingScopeCluster},
		},
	}
	if len(allow) > 0 || len(deny) > 0 {
		b.Spec.Tools = &guardrailv1alpha1.ToolsSpec{Allow: allow, Deny: deny}
	}
	return b
}
...
var _ = Describe("GuardrailBinding Controller", func() {
	...
	Describe("idempotency", func() {
		...
		It("should converge to Ready in ≤3 reconciles with no spec change", func() {
			...
			var final *guardrailv1alpha1.GuardrailBinding
			Eventually(func(g Gomega) {
				got := &guardrailv1alpha1.GuardrailBinding{}
				g.Expect(k8sClient.Get(ctx, key, got)).To(Succeed())
				g.Expect(got.Status.Phase).To(Equal(guardrailv1alpha1.BindingPhaseReady))
	...
	Describe("default-binding-missing event", func() {
		...
		It("should emit DefaultBindingMissing event for Tenant-scoped binding when default absent", func() {
			b := &guardrailv1alpha1.GuardrailBinding{
				...
				Spec: guardrailv1alpha1.GuardrailBindingSpec{
					Scope: guardrailv1alpha1.BindingScope{
						Type:      guardrailv1alpha1.BindingScopeTenant,
						TenantRef: &guardrailv1alpha1.NamespacedRef{Name: "acme", Namespace: "keese-system"},
					},
				},
			}
			Expect(k8sClient.Create(ctx, b)).To(Succeed())
	...
	Describe("strictest-wins 3-level merge", func() {
		...
		It("should compute intersection of allows and union of denies", func() {
			...
			tenant := &guardrailv1alpha1.GuardrailBinding{
				ObjectMeta: metav1.ObjectMeta{Name: tenantName, Namespace: ns, Labels: managedLabels()},
				Spec: guardrailv1alpha1.GuardrailBindingSpec{
					Scope: guardrailv1alpha1.BindingScope{Type: guardrailv1alpha1.BindingScopeTenant,
						TenantRef: &guardrailv1alpha1.NamespacedRef{Name: "acme", Namespace: ns}},
					Tools:   &guardrailv1alpha1.ToolsSpec{Allow: []string{"tool_a", "tool_b"}, Deny: []string{"bad_2"}},
					Inherit: []guardrailv1alpha1.InheritRef{{Name: clusterName, Namespace: ns}},
				},
			}
			Expect(k8sClient.Create(ctx, tenant)).To(Succeed())

			// workspace: inherits cluster then tenant (full chain in order), allow [A], deny [bad3]
			ws := &guardrailv1alpha1.GuardrailBinding{
				ObjectMeta: metav1.ObjectMeta{Name: wsName, Namespace: ns, Labels: managedLabels()},
				Spec: guardrailv1alpha1.GuardrailBindingSpec{
					Scope: guardrailv1alpha1.BindingScope{Type: guardrailv1alpha1.BindingScopeWorkspace,
						WorkspaceRef: &guardrailv1alpha1.NamespacedRef{Name: "ws1", Namespace: ns}},
					Tools: &guardrailv1alpha1.ToolsSpec{Allow: []string{"tool_a"}, Deny: []string{"bad_3"}},
					...
					Inherit: []guardrailv1alpha1.InheritRef{
	...
	Describe("TOCTOU StaleParentStatus", func() {
		...
		It("should eventually converge observedGeneration to generation after spec update", func() {
			...
			updated := &guardrailv1alpha1.GuardrailBinding{}
	...
	Describe("finalizer cascade on delete", func() {
		...
		It("should remove finalizer and call ReBAC/Kyverno delete on resource deletion", func() {
			...
			b := &guardrailv1alpha1.GuardrailBinding{
				...
				Spec: guardrailv1alpha1.GuardrailBindingSpec{
					Scope: guardrailv1alpha1.BindingScope{Type: guardrailv1alpha1.BindingScopeCluster},
					Kyverno: []guardrailv1alpha1.KyvernoPolicyRef{
						{PolicyRef: "my-policy"},
					},
					Tools: &guardrailv1alpha1.ToolsSpec{Allow: []string{"tool_a"}},
				},
			}
			Expect(k8sClient.Create(ctx, b)).To(Succeed())
