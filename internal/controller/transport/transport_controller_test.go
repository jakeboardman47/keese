...
func makeTransport(name string, spec transportv1alpha1.TransportSpec) *transportv1alpha1.Transport {
...
var _ = Describe("Transport Controller", func() {
	...
	Context("Idempotency", func() {
		...
		BeforeEach(func() {
			tr = makeTransport(resName, transportv1alpha1.TransportSpec{
				Type: transportv1alpha1.TransportTypeStdio,
				Stdio: &transportv1alpha1.StdioConfig{
					BridgeImage: "ghcr.io/keese-ai/stdio-bridge:latest",
				},
			})
			err := k8sClient.Get(ctx, types.NamespacedName{Name: resName, Namespace: testNamespace}, &transportv1alpha1.Transport{})
			if err != nil {
		...
		AfterEach(func() {
			existing := &transportv1alpha1.Transport{}
		...
		It("converges in ≤ 3 reconciles with no spec change", func() {
			...
			var lastPhase transportv1alpha1.TransportPhase
			for i := 0; i < 3; i++ {
				_, err := reconcileOnce(ctx, r, resName)
				Expect(err).NotTo(HaveOccurred())

				got := &transportv1alpha1.Transport{}
			...
			Expect(lastPhase).To(Equal(transportv1alpha1.TransportPhaseReady))
		})
	})
	...
	Context("NATS hybrid ownership — default mode", func() {
		...
		It("sets Degraded with NATSStreamNotFound when stream is absent", func() {
			tr := makeTransport(resName, transportv1alpha1.TransportSpec{
				Type: transportv1alpha1.TransportTypeNATS,
				NATS: &transportv1alpha1.NATSConfig{
					ClusterRef:   transportv1alpha1.NamespacedObjectRef{Name: "nats-cluster"},
			...
			got := &transportv1alpha1.Transport{}
			Expect(k8sClient.Get(ctx, types.NamespacedName{Name: resName, Namespace: testNamespace}, got)).To(Succeed())
			Expect(got.Status.Phase).To(Equal(transportv1alpha1.TransportPhaseDegraded))
	...
	Context("NATS hybrid ownership — opt-in mode", func() {
		...
		It("creates the stream and adds finalizer", func() {
			...
			got := &transportv1alpha1.Transport{}
			Expect(k8sClient.Get(ctx, types.NamespacedName{Name: resName, Namespace: testNamespace}, got)).To(Succeed())
			Expect(got.Status.Phase).To(Equal(transportv1alpha1.TransportPhaseReady))
			Expect(got.Finalizers).To(ContainElement(transportFinalizer))
		})

		It("deletes the owned stream on cleanup", func() {
			tr := makeTransport(resName+"-del", transportv1alpha1.TransportSpec{
				Type: transportv1alpha1.TransportTypeNATS,
				NATS: &transportv1alpha1.NATSConfig{
					ClusterRef:   transportv1alpha1.NamespacedObjectRef{Name: "nats-cluster"},
			...
			obj := &transportv1alpha1.Transport{}
	...
	Context("MCP transport MCPRoute validation", func() {
		...
		It("degrades with MCPRouteNotFound when the route is absent", func() {
			tr := makeTransport(resName, transportv1alpha1.TransportSpec{
				Type: transportv1alpha1.TransportTypeMCP,
				MCP: &transportv1alpha1.MCPConfig{
					McpRouteRef:     transportv1alpha1.NamespacedObjectRef{Name: "missing-route"},
		...
		It("degrades with CertificateNotFound when cert is absent", func() {
			tr := makeTransport(resName, transportv1alpha1.TransportSpec{
				...
				NATS: &transportv1alpha1.NATSConfig{
					...
					ConsumerName: "my-consumer",
					TLS: &transportv1alpha1.NATSTLSConfig{
						CertificateRef: &transportv1alpha1.NamespacedObjectRef{
							Name:      "missing-cert",
							Namespace: testNamespace,
						},
					},
	...
	Context("A2A cross-tenant scope", func() {
		...
		It("degrades with CrossTenantAgreementMissing without an Approved CTA", func() {
			scope := transportv1alpha1.A2AScopeCrossTenant
			auth := transportv1alpha1.A2APeerAuthWorkspaceSA
			tr := makeTransport(resNameReject, transportv1alpha1.TransportSpec{
				Type: transportv1alpha1.TransportTypeA2A,
				A2A: &transportv1alpha1.A2AConfig{
					Endpoint: "grpcs://peer.tenant-b.svc:8443",
					PeerAuth: &auth,
					Scope:    &scope,
					WorkspaceSA: &transportv1alpha1.WorkspaceSAConfig{
		...
		It("provisions successfully with an Approved CTA", func() {
			...
			auth := transportv1alpha1.A2APeerAuthWorkspaceSA
			endpoint := "grpcs://peer.tenant-b.svc:8443"
			tr := makeTransport(resNameAccept, transportv1alpha1.TransportSpec{
				Type: transportv1alpha1.TransportTypeA2A,
				A2A: &transportv1alpha1.A2AConfig{
					Endpoint: endpoint,
					PeerAuth: &auth,
...
func (r *mcpNotFoundReconciler) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	...
	if tr.Spec.Type == transportv1alpha1.TransportTypeMCP && tr.Spec.MCP != nil {
