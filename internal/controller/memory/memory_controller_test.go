...
var _ = Describe("Memory Controller", func() {
	...
	buildMemory := func(name string) *memoryv1alpha1.Memory {
		return &memoryv1alpha1.Memory{
			ObjectMeta: metav1.ObjectMeta{
				Name:      name,
				Namespace: memTestNS,
			},
			Spec: memoryv1alpha1.MemorySpec{
	...
	Describe("happy path — SQLite provider", func() {
		...
		It("transitions to Ready phase", func() {
			...
			Eventually(func(g Gomega) {
				...
				g.Expect(mem.Status.Phase).To(Equal(memoryv1alpha1.MemoryPhaseReady))
		...
		It("is idempotent across ≥3 reconciles", func() {
			...
			Expect(mem.Status.Phase).To(Equal(memoryv1alpha1.MemoryPhaseReady))
		})
	})
		...
		It("sets Degraded phase when redis replicas < 2 in non-dev namespace", func() {
			mem := &memoryv1alpha1.Memory{
				...
				Spec: memoryv1alpha1.MemorySpec{
					...
					Provider: memoryv1alpha1.MemoryProvider{
						Type: memoryv1alpha1.ProviderRedis,
						Redis: &memoryv1alpha1.RedisConfig{
							Address:  "redis:6379",
		...
		It("accepts mem0 provider spec without error", func() {
			mem := &memoryv1alpha1.Memory{
				...
				Spec: memoryv1alpha1.MemorySpec{
					...
					Provider: memoryv1alpha1.MemoryProvider{
						Type: memoryv1alpha1.ProviderMem0,
						Mem0: &memoryv1alpha1.Mem0Config{
