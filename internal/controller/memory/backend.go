...
type BackendProvisioner interface {
	...
	Provision(ctx context.Context, provider memoryv1alpha1.MemoryProvider, name, namespace string) (created bool, err error)

	// Deprovision removes the backend resource.
	// Missing resources are silently ignored (idempotent).
	Deprovision(ctx context.Context, provider memoryv1alpha1.MemoryProvider, name, namespace string) error

	// Healthy returns true if the backend can be reached.
	// Called during every reconcile to drive phase transitions.
	Healthy(ctx context.Context, provider memoryv1alpha1.MemoryProvider, name, namespace string) (bool, error)
}
...
func (f *FakeBackendProvisioner) Provision(_ context.Context, _ memoryv1alpha1.MemoryProvider, name, namespace string) (bool, error) {
...
func (f *FakeBackendProvisioner) Deprovision(_ context.Context, _ memoryv1alpha1.MemoryProvider, name, namespace string) error {
...
func (f *FakeBackendProvisioner) Healthy(_ context.Context, _ memoryv1alpha1.MemoryProvider, name, namespace string) (bool, error) {
	if f.HealthyErr != nil {
...
func validateHA(provider memoryv1alpha1.MemoryProvider, namespace string) error {
	if isDevNamespace(namespace) {
		return nil
	}
	switch provider.Type {
	case memoryv1alpha1.ProviderRedis:
	...
	case memoryv1alpha1.ProviderQdrant:
