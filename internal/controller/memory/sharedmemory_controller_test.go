...
var _ = Describe("SharedMemory Controller", func() {
	buildSM := func(name string, sharedWith ...memoryv1alpha1.WorkspaceRef) *memoryv1alpha1.SharedMemory {
		return &memoryv1alpha1.SharedMemory{
			ObjectMeta: metav1.ObjectMeta{
				Name:      name,
				Namespace: smTestNS,
			},
			Spec: memoryv1alpha1.SharedMemorySpec{
				TenantRef: "tenant-" + name,
				Provider: memoryv1alpha1.MemoryProvider{
					Type:   memoryv1alpha1.ProviderSQLite,
					SQLite: &memoryv1alpha1.SQLiteConfig{StorageSize: "1Gi"},
				},
				EmbeddingDim: 768,
	...
	Describe("happy path — no sharedWith", func() {
		var sm *memoryv1alpha1.SharedMemory
		...
		It("reaches Ready phase", func() {
			...
			Eventually(func(g Gomega) {
				...
				g.Expect(sm.Status.Phase).To(Equal(memoryv1alpha1.MemoryPhaseReady))
	...
	Describe("sharedWith reader/writer tuple sync", func() {
		It("writes reader and writer tuples for sharedWith entries", func() {
			sm := buildSM("sm-shared-rw",
				memoryv1alpha1.WorkspaceRef{Name: "ws-reader", Namespace: "ns-a", Access: "reader"},
				memoryv1alpha1.WorkspaceRef{Name: "ws-writer", Namespace: "ns-b", Access: "writer"},
			)
			Expect(k8sClient.Create(ctx, sm)).To(Succeed())
			nn := types.NamespacedName{Name: sm.Name, Namespace: smTestNS}
	...
	Describe("finalizer tuple-purge-before-deprovision order", func() {
		It("purges all shared tuples before backend deprovision", func() {
			sm := buildSM("sm-finalize-order",
				memoryv1alpha1.WorkspaceRef{Name: "ws-del", Namespace: "ns-del", Access: "reader"},
			)
			Expect(k8sClient.Create(ctx, sm)).To(Succeed())
			nn := types.NamespacedName{Name: sm.Name, Namespace: smTestNS}
	...
	Describe("HA validation — qdrant outside dev namespace", func() {
		It("rejects qdrant with replicas < 2 in non-dev namespace", func() {
			provider := memoryv1alpha1.MemoryProvider{
				Type: memoryv1alpha1.ProviderQdrant,
				Qdrant: &memoryv1alpha1.QdrantConfig{
					CollectionName: "test",
	...
	Describe("idempotency across ≥3 reconciles", func() {
		It("converges with no spec change", func() {
			sm := buildSM("sm-idempotent",
				memoryv1alpha1.WorkspaceRef{Name: "ws-idem", Namespace: "ns-idem", Access: "reader"},
			)
			Expect(k8sClient.Create(ctx, sm)).To(Succeed())
			nn := types.NamespacedName{Name: sm.Name, Namespace: smTestNS}
			...
			Expect(sm.Status.Phase).To(Equal(memoryv1alpha1.MemoryPhaseReady))
