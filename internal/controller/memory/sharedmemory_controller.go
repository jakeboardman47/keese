...
func (r *SharedMemoryReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	sm := &memoryv1alpha1.SharedMemory{}
	...
	if err := validateHA(sm.Spec.Provider, sm.Namespace); err != nil {
		...
		return r.setSharedStatus(ctx, sm, orig, memoryv1alpha1.MemoryPhaseDegraded,
			metav1.Condition{
				Type:    "Ready",
	...
	sm.Status.Phase = memoryv1alpha1.MemoryPhaseProvisioning
	...
	return r.setSharedStatus(ctx, sm, orig, memoryv1alpha1.MemoryPhaseReady,
		metav1.Condition{
			Type:    "Ready",
...
func (r *SharedMemoryReconciler) buildSharedTuples(sm *memoryv1alpha1.SharedMemory) ([]Tuple, error) {
	id := string(sm.UID)
	tuples := make([]Tuple, 0, len(sm.Spec.SharedWith))
...
func (r *SharedMemoryReconciler) reconcileDelete(ctx context.Context, sm, orig *memoryv1alpha1.SharedMemory) (ctrl.Result, error) {
	log := logf.FromContext(ctx)
	...
	sm.Status.Phase = memoryv1alpha1.MemoryPhaseTerminating
	...
	if len(tuples) > 0 {
		if err := r.Rebac.Delete(ctx, tuples); err != nil {
			...
			return r.setSharedStatus(ctx, sm, orig, memoryv1alpha1.MemoryPhaseTerminating,
				metav1.Condition{
					Type:    "Ready",
					Status:  metav1.ConditionFalse,
...
func (r *SharedMemoryReconciler) setSharedStatus(
	ctx context.Context,
	sm, orig *memoryv1alpha1.SharedMemory,
...
func (r *SharedMemoryReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&memoryv1alpha1.SharedMemory{}).
