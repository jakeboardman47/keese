...
const (
	...
	// managedLabel is the predicate label required on all managed resources.
...
func (r *RecipeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if recipe.Status.Phase == "" {
		recipe.Status.Phase = recipev1alpha1.RecipePhasePending
	}

	// Transition to Pulling.
	recipe.Status.Phase = recipev1alpha1.RecipePhasePulling
	setCondition(&recipe.Status.Conditions, metav1.Condition{
		Type:               recipev1alpha1.RecipeConditionProgressing,
	...
	if err := r.Get(ctx, rsKey, &rs); err != nil {
		...
		recipe.Status.Phase = recipev1alpha1.RecipePhaseFailed
		setCondition(&recipe.Status.Conditions, metav1.Condition{
			Type:               recipev1alpha1.RecipeConditionReady,
	...
	if rs.Status.Phase != recipev1alpha1.RecipeSourcePhaseSynced || !rs.Status.Cached {
	...
	recipe.Status.Phase = recipev1alpha1.RecipePhaseVerified
	...
	// Advance to Ready.
	recipe.Status.Phase = recipev1alpha1.RecipePhaseReady
	recipe.Status.ObservedGeneration = recipe.Generation
	...
	setCondition(&recipe.Status.Conditions, metav1.Condition{
		Type:               recipev1alpha1.RecipeConditionVerified,
		Status:             metav1.ConditionTrue,
		Reason:             "Verified",
...
func (r *RecipeReconciler) cleanup(ctx context.Context, recipe *recipev1alpha1.Recipe, orig *recipev1alpha1.Recipe) (ctrl.Result, error) {
	log := logf.FromContext(ctx)
	...
	recipe.Status.Phase = recipev1alpha1.RecipePhaseTerminating
...
func (r *RecipeReconciler) patchStatus(ctx context.Context, recipe, orig *recipev1alpha1.Recipe) error {
...
func recipeRebacTuples(recipe *recipev1alpha1.Recipe) []RebacTuple {
