...
func (r *CrossTenantAgreementReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if cra.Status.Phase == "" {
		cra.Status.Phase = tenancyv1alpha1.CRAPhaseP
	}

	// Terminal phases: Rejected and Expired are append-only; no further state changes.
	if cra.Status.Phase == tenancyv1alpha1.CRAPhaseR || cra.Status.Phase == tenancyv1alpha1.CRAPhaseE {
	...
	if cra.Status.Phase == tenancyv1alpha1.CRAPhaseA && cra.Spec.ExpiresAt != "" {
	...
	if cra.Status.Phase == tenancyv1alpha1.CRAPhaseP && r.bothTenantsApproved(&cra) {
	...
	if cra.Status.Phase == tenancyv1alpha1.CRAPhaseP && len(cra.Status.Approvals) == 0 {
		if conflict, err := r.detectConflict(ctx, &cra); err != nil {
			...
			cra.Status.Phase = tenancyv1alpha1.CRAPhaseR
	...
	result := ctrl.Result{}
	if cra.Spec.ExpiresAt != "" && cra.Status.Phase == tenancyv1alpha1.CRAPhaseA {
...
func (r *CrossTenantAgreementReconciler) validateApprovalAnnotation(ctx context.Context, cra *tenancyv1alpha1.CrossTenantAgreement) *tenancyv1alpha1.CRAApproval {
	...
	// Verify signature.
	var verifyErr error
	st := tenancyv1alpha1.SignatureTypeSAToken
	if sigType == string(tenancyv1alpha1.SignatureTypeOIDCKeyless) {
		st = tenancyv1alpha1.SignatureTypeOIDCKeyless
...
func (r *CrossTenantAgreementReconciler) bothTenantsApproved(cra *tenancyv1alpha1.CrossTenantAgreement) bool {
...
func (r *CrossTenantAgreementReconciler) transitionToApproved(ctx context.Context, cra *tenancyv1alpha1.CrossTenantAgreement, orig *tenancyv1alpha1.CrossTenantAgreement) (ctrl.Result, error) {
	log := logf.FromContext(ctx)
	...
	now := metav1.Now()
	snapshot := make([]tenancyv1alpha1.WorkspaceSnapshotEntry, 0, len(fromWorkspaces)*len(toWorkspaces))
	for _, from := range fromWorkspaces {
		for _, to := range toWorkspaces {
			snapshot = append(snapshot, tenancyv1alpha1.WorkspaceSnapshotEntry{
				FromWorkspace: from,
				ToWorkspace:   to,
				SnapshotAt:    now,
			})
	...
	// Write ReBAC tuples.
	tuples := []RebacTuple{craAllowsMessagingTuple(cra.Spec.From.TenantRef.Name, cra.Spec.To.TenantRef.Name)}
...
func (r *CrossTenantAgreementReconciler) transitionToExpired(ctx context.Context, cra *tenancyv1alpha1.CrossTenantAgreement, orig *tenancyv1alpha1.CrossTenantAgreement) (ctrl.Result, error) {
	log := logf.FromContext(ctx)
	...
	cra.Status.Phase = tenancyv1alpha1.CRAPhaseE
	r.setCRACondition(&cra.Status.Conditions, metav1.Condition{
		Type:               "Ready",
		Status:             metav1.ConditionFalse,
		Reason:             "Expired",
...
func (r *CrossTenantAgreementReconciler) checkSnapshotDrift(ctx context.Context, cra *tenancyv1alpha1.CrossTenantAgreement) {
...
func (r *CrossTenantAgreementReconciler) detectConflict(ctx context.Context, cra *tenancyv1alpha1.CrossTenantAgreement) (bool, error) {
	...
	for i := range craList.Items {
		...
		if other.Status.Phase != tenancyv1alpha1.CRAPhaseA {
			continue
		}
...
func (r *CrossTenantAgreementReconciler) cleanup(ctx context.Context, cra *tenancyv1alpha1.CrossTenantAgreement, orig *tenancyv1alpha1.CrossTenantAgreement) (ctrl.Result, error) {
	log := logf.FromContext(ctx)
...
func (r *CrossTenantAgreementReconciler) isExpired(cra *tenancyv1alpha1.CrossTenantAgreement) (bool, error) {
	if cra.Spec.ExpiresAt == "" {
		return false, nil
	}
...
func (r *CrossTenantAgreementReconciler) patchCRAStatus(ctx context.Context, cra *tenancyv1alpha1.CrossTenantAgreement, orig *tenancyv1alpha1.CrossTenantAgreement) error {
...
func (r *CrossTenantAgreementReconciler) setCRACondition(conditions *[]metav1.Condition, c metav1.Condition) {
	setTenantCondition(conditions, c)
}
