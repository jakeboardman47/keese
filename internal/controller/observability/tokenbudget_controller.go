...
func (r *TokenBudgetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if parseErr == nil && now.After(windowEnd) {
		if err := r.resetWindow(ctx, &tb, orig, now); err != nil {
			...
			tb.Status.Phase = observabilityv1alpha1.TokenBudgetPhaseResetFailed
	...
	metricHealthy := true
	newConsumed := make([]observabilityv1alpha1.TokenUsageEntry, 0, len(tb.Spec.Limits))
	...
	for _, limit := range tb.Spec.Limits {
		...
		newConsumed = append(newConsumed, observabilityv1alpha1.TokenUsageEntry{
			Model:        limit.Model,
	...
	for i, limit := range tb.Spec.Limits {
		...
		if exceeded {
			anyExceeded = true
			if tb.Spec.ExhaustionMode == observabilityv1alpha1.ExhaustionModeHard {
	...
	// Update phase.
	switch {
	case anyExceeded && tb.Spec.ExhaustionMode == observabilityv1alpha1.ExhaustionModeHard:
		tb.Status.Phase = observabilityv1alpha1.TokenBudgetPhaseExhausted
	case anyExceeded && tb.Spec.ExhaustionMode == observabilityv1alpha1.ExhaustionModeSoft:
		tb.Status.Phase = observabilityv1alpha1.TokenBudgetPhaseSoftExhausted
	default:
		if tb.Status.Phase != observabilityv1alpha1.TokenBudgetPhaseReady {
			r.Recorder.Eventf(&tb, corev1.EventTypeNormal, ReasonBudgetActive,
				"TokenBudget %s/%s is Ready", tb.Namespace, tb.Name)
		}
		tb.Status.Phase = observabilityv1alpha1.TokenBudgetPhaseReady
	}
	...
	setCondition(&tb.Status.Conditions, metav1.Condition{
		Type:               "Ready",
		Status:             boolCondStatus(!anyExceeded || tb.Spec.ExhaustionMode == observabilityv1alpha1.ExhaustionModeDisabled),
...
func (r *TokenBudgetReconciler) cleanup(ctx context.Context, tb *observabilityv1alpha1.TokenBudget, orig *observabilityv1alpha1.TokenBudget) (ctrl.Result, error) {
	log := logf.FromContext(ctx)
...
func (r *TokenBudgetReconciler) initWindow(ctx context.Context, tb *observabilityv1alpha1.TokenBudget, orig *observabilityv1alpha1.TokenBudget, now time.Time) error {
...
func (r *TokenBudgetReconciler) resetWindow(ctx context.Context, tb *observabilityv1alpha1.TokenBudget, orig *observabilityv1alpha1.TokenBudget, now time.Time) error {
	windowDur, err := parseWindowDuration(tb.Spec.WindowDuration)
	if err != nil {
...
func (r *TokenBudgetReconciler) queryConsumed(ctx context.Context, tb *observabilityv1alpha1.TokenBudget, scopeType, scopeID, model string) (observabilityv1alpha1.TokenUsageEntry, error) {
	...
	if err != nil {
		return observabilityv1alpha1.TokenUsageEntry{Model: model}, err
	}
	...
	return observabilityv1alpha1.TokenUsageEntry{
		Model:        model,
...
// meets or exceeds it.
func (r *TokenBudgetReconciler) isExceeded(limit observabilityv1alpha1.TokenLimit, consumed observabilityv1alpha1.TokenUsageEntry) bool {
...
func (r *TokenBudgetReconciler) computeRemaining(limit observabilityv1alpha1.TokenLimit, consumed observabilityv1alpha1.TokenUsageEntry) int64 {
...
func (r *TokenBudgetReconciler) resolveScope(tb *observabilityv1alpha1.TokenBudget) (string, string) {
...
func (r *TokenBudgetReconciler) checkCardinality(ctx context.Context, tb *observabilityv1alpha1.TokenBudget) error {
	var list observabilityv1alpha1.TokenBudgetList
...
func (r *TokenBudgetReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(&observabilityv1alpha1.TokenBudget{}).
...
func findConsumed(entries []observabilityv1alpha1.TokenUsageEntry, model string) observabilityv1alpha1.TokenUsageEntry {
	for _, e := range entries {
		if e.Model == model {
			return e
		}
	}
	return observabilityv1alpha1.TokenUsageEntry{Model: model}
}
