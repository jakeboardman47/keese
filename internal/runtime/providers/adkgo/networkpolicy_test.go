...
// See go_provider_test.go for why these pure-render tests carry no build tag
// (default unit tier, keeps the coverage floor honest).
...
func testNetworkPolicyInput() NetworkPolicyInput {
	return NetworkPolicyInput{
		Name:          "keese-ws-uid-1234-adk-go-netpol",
		Namespace:     "default",
...
// TestADKGoProvider_NetworkPolicy asserts the fail-closed network posture of the
// rendered policy: default-deny BOTH directions (rule 04.17 + 05.4), each EXACT
// allow present (gateway:443, NATS:4222, peer A2A ingress/egress :8081), and NO
// wildcard anywhere (rule 05.5 — no empty podSelector:{} with open to:[], no
// open ipBlock). Mirrors TestADKPythonProvider_NetworkPolicy.
func TestADKGoProvider_NetworkPolicy(t *testing.T) {
...
// TestADKGoProvider_NetworkPolicyIdempotency asserts the NetworkPolicy render is
// a pure function of its input: three successive BuildNetworkPolicy calls with
// identical input produce deeply-equal objects (SSA × 3, no churn — rule 06.6),
// and that the adkGo policy is rendered under its own distinct object name so it
// never conflicts with the adkPython policy at SSA time (the controller applies
// each under a uniquely-named object: keese-ws-<uid>-adk-go-netpol vs
// keese-ws-<uid>-adk-netpol).
func TestADKGoProvider_NetworkPolicyIdempotency(t *testing.T) {
	...
	// No-conflict-with-Python guard: the adkGo policy carries the adk-go object
	// name, never the adkPython name. A workspace that switches runtimes gets a
	// new object rather than an SSA fieldOwner fight over a shared name.
	if first.Name != "keese-ws-uid-1234-adk-go-netpol" {
		t.Errorf("adkGo policy name: got %q, want the adk-go-specific name", first.Name)
	}
}

// TestADKGoProvider_NetworkPolicyInputFromCRs asserts the CR→input marshal
// carries the workspace name, namespace, deterministic policy name, and
// provenance labels through to the NetworkPolicyInput the controller feeds
// BuildNetworkPolicy.
func TestADKGoProvider_NetworkPolicyInputFromCRs(t *testing.T) {
	ws := &keesev1alpha1.Workspace{}
	ws.Name = "ws-demo"

	labels := map[string]string{"keese.ai/workspace": "ws-demo"}
	in := NetworkPolicyInputFromCRs(ws, "tenant-ns", "keese-ws-uid-1234-adk-go-netpol", labels)

	if in.WorkspaceName != "ws-demo" {
		t.Errorf("workspace: got %q, want ws-demo", in.WorkspaceName)
	}
	if in.Namespace != "tenant-ns" {
		t.Errorf("namespace: got %q, want tenant-ns", in.Namespace)
	}
	if in.Name != "keese-ws-uid-1234-adk-go-netpol" {
		t.Errorf("name: got %q", in.Name)
	}
	if !reflect.DeepEqual(in.Labels, labels) {
		t.Errorf("labels: got %+v, want %+v", in.Labels, labels)
	}

	// The marshaled input must render a policy scoped to this workspace.
	np := BuildNetworkPolicy(in)
	if np.Spec.PodSelector.MatchLabels["keese.ai/workspace"] != "ws-demo" {
		t.Errorf("rendered policy podSelector: got %+v", np.Spec.PodSelector.MatchLabels)
	}
}

// hasEgressRule reports whether np has an egress rule allowing tcp/port to a peer
// selected by (namespace metadata.name == ns) AND (pod label key==val).
