...
// E3 T4 — fail-closed NetworkPolicy for the ADK Go session pod.
//
// Mirrors the adkPython sibling (internal/runtime/providers/adkpython) exactly:
// the renderer is duplicated rather than shared because the adkPython
// equivalents are package-private and extracting a shared builder would mean
// editing the adkPython package (the E3 plan permits duplication when the
// alternative is a non-trivial cross-package refactor). The two policies are
// byte-identical in shape; only the workspace label selector differs per call.
//
// The WorkspaceSession reconciler SSA-applies this policy ONLY on the adkGo
// branch (applySessionNetworkPolicy). The goose and adkPython paths are
// untouched. This policy is a pure function of its inputs so SSA re-applies are
...
//     is covered by TestADKGoProvider_NetworkPolicy.
//
...
const (
	...
	// (goose, adkPython, adkGo) carries via sessionPodObjectMeta.
...
// BuildNetworkPolicy renders the fail-closed ADK Go NetworkPolicy.
//
