...
// E3 T2 — single-container ADK Go pod template.
//
// The WorkspaceSession reconciler (T5 discriminator) calls BuildPodSpec when
// AgentRuntime.spec.implementation.adkGo is set, instead of the goose or
// adkPython pod path. The render mirrors the adkPython template
// (internal/runtime/providers/adkpython) EXACTLY for security posture; the only
// differences are the single static Go binary command line and the
// distroless/static nonroot identity. The render is a pure function of its
// inputs so it is exhaustively unit-/envtest-able without a live cluster
// (TestADKGoProvider_PodRender) and so SSA re-applies are byte-stable across
// reconciles (3-reconcile idempotency).
//
// Security invariants enforced here (do not relax without an ADR) — identical
// to the adkPython sibling:
...
const (
	// ContainerName is the ADK Go container's name in the pod.
	ContainerName = "adk-go"

	// BridgeContainerName is the A2A bridge sidecar's name (reused from E1 T3).
	BridgeContainerName = "a2a-bridge"

	// A2APort is the in-pod port the ADK Go server listens on. It is
	// localhost-only: the a2a-bridge sidecar (E1 T3, reused here) is the sole
	// peer-facing ingress (A2ABridgePort) and forwards inbound A2A traffic here.
	A2APort = 8080
	...
	// localhost. The NetworkPolicy (T4) gates ingress at exactly this port.
	A2ABridgePort = 8081
	...
	// Same image as the adkPython provider — the bridge binary is runtime-agnostic.
...
type PodInput struct {
	// Image is the ADK Go OCI reference (ADKGoSpec.Image).
	Image string
...
// BuildPodSpec renders the single-container ADK Go pod spec.
//
...
// credentials. Identical posture to the adkPython sibling.
func BuildPodSpec(in PodInput) corev1.PodSpec {
	...
	return corev1.PodSpec{
		...
		SecurityContext: &corev1.PodSecurityContext{
			...
			// Distroless static nonroot uid (gcr.io/distroless/static-debian12:nonroot).
...
// adkContainer renders the primary ADK Go runtime container. The ADK server
...
//
// The command is the single static binary baked into Dockerfile.adk-go
// (/app/adk-go), in contrast to the adkPython sibling's interpreter + module
// invocation. Same gateway-routing arg so model traffic egresses the gateway.
func adkContainer(in PodInput) corev1.Container {
	return corev1.Container{
		Name:            ContainerName,
		Image:           in.Image,
		ImagePullPolicy: corev1.PullIfNotPresent,
		Command:         []string{"/app/adk-go"},
		Args: []string{
			"serve",
		...
		// preStop drain: the adk-go binary checkpoints session state to the PVC
		// before the kubelet deletes the pod (rule 06.2). Unlike the Python
		// image (which bundles a separate /usr/local/bin/keese-drain), the Go
		// runtime self-drains via its own `drain` subcommand — distroless/static
		// carries no second binary.
...
// bridgeContainer renders the a2a-bridge sidecar (reused from E1 T3 — NO new
// code; the same runtime-agnostic bridge image). It listens on A2ABridgePort
// for inbound A2A traffic from peer workspaces and forwards to the ADK server
// on localhost:A2APort, reading the projected MCP ConfigMap.
//
...
// localhost traffic and a read-only projected ConfigMap. The upstream forward
// target (localhost:A2APort, i.e. RUNTIME_A2A_UPSTREAM_PORT=8080) is a
// compile-time constant in internal/runtime/a2a/bridge, so no env is needed.
func bridgeContainer(in PodInput) corev1.Container {
	...
	return corev1.Container{
		...
		// trivially true and asserted by TestADKGoProvider_PodRender.
...
// every container in the ADK Go pod: non-root, read-only root filesystem,
...
//
// Invariant asserted by TestADKGoProvider_PodRender: no env name or value
...
func podVolumes(in PodInput, tokenExpiry int64) []corev1.Volume {
	return []corev1.Volume{
		...
		{
			// MCP server list for the a2a-bridge sidecar (E1 T3, reused).
			// Rendered by the GuardrailBinding reconciler (E6); Optional so a
			// missing ConfigMap never blocks pod creation before E6 ships — the
			// bridge treats a missing/empty config.json as non-fatal.
			Name: "mcp-config",
			VolumeSource: corev1.VolumeSource{
				ConfigMap: &corev1.ConfigMapVolumeSource{
					LocalObjectReference: corev1.LocalObjectReference{
...
func PodInputFromCRs(
	...
	return PodInput{
		Image:              ar.Spec.Implementation.AdkGo.Image,
