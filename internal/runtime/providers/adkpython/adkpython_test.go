...
// TestRegistration asserts the package init() registered the adkPython
// provider under the documented name, that Lookup returns the zeroed E0
// capability matrix, and that the factory builds a Runtime answering to the
// same name.
func TestRegistration(t *testing.T) {
	...
	rt, err := factory(map[string]string{"image": "ghcr.io/keese/adk-python:test"})
	if err != nil {
...
// optional features (all flags false / zero) per the adkPython skeleton design.
