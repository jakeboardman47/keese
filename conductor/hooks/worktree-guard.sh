...
#   {"type":"command","command":"${CLAUDE_PROJECT_DIR}/conductor/hooks/worktree-guard.sh"}

set -euo pipefail

input="$(cat)"

# Enforce only for conductor-dispatched (bypassPermissions) agents — they carry
# CONDUCT_PHASE_ID. The main interactive session and normally-permissioned chat
# subagents are already governed by the allow/ask/deny matrix, and may legitimately
# touch sibling repos / additional working dirs. Set CONDUCT_GUARD_ALL=1 to enforce
# everywhere.
[[ -n "${CONDUCT_PHASE_ID:-}" || "${CONDUCT_GUARD_ALL:-0}" == "1" ]] || exit 0
