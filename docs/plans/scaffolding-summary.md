...
| P6 operator scaffold | `5dd8d07` | operator-sdk init + create-api for 13 kinds at v1alpha1 (original 8-group layout, since consolidated to the 3 `keese.ai` groups by TD-P1-03) — every `*_types.go` + `*_controller.go` is a `TODO(design-gate)` stub (26 markers) — OLM bundle validate green |
...
- Design gate: **CLOSED** per `docs/plans/README.md`
  ...
  until all 62 designs + 13 specs score ≥ 90/100 on the rubric and
...
design authoring** — walk the 48 design stubs in
...
Once all 62 designs are `current`, the 13 specs under `docs/specs/`
are authored (each depends on 1–3 designs). When the 13 specs reach
